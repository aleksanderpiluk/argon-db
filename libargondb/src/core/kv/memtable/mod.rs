...
#[async_trait]
impl scannable::Scannable for Memtable {
    async fn range_scan(&self, params: scannable::RangeScanParams) -> Result<(), ()> {
        skip_list::ScanIter::for_range_scan(&self.inner, params);
        todo!()
    }

    async fn row_scan(&self, params: scannable::RowScanParams) -> Result<(), ()> {
        skip_list::ScanIter::for_row_scan(&self.inner, params);
        todo!()
    }
}
